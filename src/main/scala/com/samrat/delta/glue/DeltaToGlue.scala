...
object DeltaToGlue {
  ...
  def deltaToGlue(
    ...
    Try {
      ...
      val msc = HiveUtils.getMSC(spark).get
      ...
      try {
        ...
        case NonFatal(e) =>
          catalog.dropTable(dbName, tableName, true, true)
          catalog.createTable(catalogTable, ignoreIfExists = true)
      }
  ...
  //  /** Take Delta table name and Athena table name as an input, Create a athena table if not exist. if athena table
  //   * exist attempts to update schema of the table with new delta table schema. Also updates the newly added partitions
  //   * to athena table incrementally.
  //   * @param spark Spark Session
  //   * @param deltaTableName Delta Table name (ex: database.tablename)
  //   * @param athenaTableName Athena table name to be created (athena_database.tablename)
  //   * @param mergeSchema provide false if, no merge schema to athena table is required, default true
  //   * @param partitionColumns Seq of partion columns
  //   */
  //  def deltaToAthena(
  //                     spark: SparkSession,
  ...
  //
  //      // Normalize path so that paths end with /
  //      val catalog = spark.sessionState.catalog.externalCatalog
  //      val tbDetails = getTableDetails(deltaTableName)
  //      val normalizedTablePathS3: String = catalog
  //        .getTable(tbDetails._1, tbDetails._2)
  //        .storage
  //        .locationUri
  //        .get
  ...
  //      }
  //
  //      val (dbName, tableName) = getTableDetails(athenaTableName)
  //
  //      if(!dbName.startsWith("athena")){
  //        throw new Exception(s"$dbName must prefix with athena")
  //      }
  //
  //      val partitionColumnNames: Seq[String] = partitionColumns.getOrElse(
  //        spark
  ...
  //      )
  //
  //      // Create Athena table if not exist in catalog or update schema with new schema if mergeSchema option is true
  //      AthenaUtils.createOrUpdate(spark = spark,
  //        path = normalizedTablePathS3,
  //        database = dbName,
  //        table = tableName,
  //        partitionColumns = partitionColumnNames,
  //        mergeSchema = mergeSchema)
  //
  //      // Add recently updated delta partitions to Athena table
  //      AthenaUtils.updateTablePartitions(spark = spark,
  //        path = normalizedTablePathS3,
  //        database = dbName,
  //        table = tableName,
  //        partitionColumns = partitionColumnNames)
  ...
  private def toHivePartition(
    ...
    Try {
      ...
      val ht = HiveUtils.toHiveTable(spark, ct).get
